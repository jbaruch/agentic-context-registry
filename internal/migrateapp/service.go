...
func (service *Service) Migrate(ctx context.Context, projectDirectory string, options Options) (migrate.MigrationReport, error) {
	...
	realized, err := service.realizer.RunState(ctx, projectDirectory, desired, desired.Project.Agents, mode)
	if err != nil {
		return migrate.MigrationReport{}, err
	}
	report, err := service.buildReport(ctx, projectDirectory, inventory, mappings, desired, realized, options.DryRun || options.Finalize)
	if err != nil {
		return migrate.MigrationReport{}, err
	}
	if options.Finalize {
...
func (service *Service) resolveState(ctx context.Context, existing dependency.State, mappings []migrate.Mapping) (dependency.State, []migrate.Mapping, error) {
	...
	for index := range mappings {
		...
		requested, locked, reused, err := service.resolveMapping(ctx, existing, *mapping)
		if err != nil {
...
func (service *Service) resolveMapping(ctx context.Context, existing dependency.State, mapping migrate.Mapping) (string, dependency.LockedDependency, bool, error) {
	...
	if mapping.Explicit {
		if hasDeclaration && hasLock && declaration.Requested == mapping.Requested && locked.Requested == mapping.Requested {
			return mapping.Requested, locked, true, nil
		}
		return mapping.Requested, dependency.LockedDependency{}, false, nil
	}
	if mapping.TesslVersion == "latest" || mapping.TesslVersion == "" {
		if hasDeclaration && hasLock && declaration.Requested == "latest" && locked.Requested == "latest" {
			return "latest", locked, true, nil
		}
		return "latest", dependency.LockedDependency{}, false, nil
	}
	if hasDeclaration && hasLock && declaration.Requested == locked.Requested && dependency.TagMatchesVersion(locked.Tag, mapping.TesslVersion) {
		return declaration.Requested, locked, true, nil
	}
	repository, err := dependency.ParseSource(mapping.Source)
	if err != nil {
		return "", dependency.LockedDependency{}, false, err
	}
	...
	if plainErr != nil && !isNotFound(plainErr) {
		return "", dependency.LockedDependency{}, false, plainErr
	}
	if prefixedErr != nil && !isNotFound(prefixedErr) {
		return "", dependency.LockedDependency{}, false, prefixedErr
	}
	if plainFound == prefixedFound {
		...
		return "", dependency.LockedDependency{}, false, namedError(code, message, nil)
	}
	if prefixedFound {
		return prefixed.Tag, dependency.LockedDependency{}, false, nil
	}
	return plain.Tag, dependency.LockedDependency{}, false, nil
}
...
func (service *Service) buildReport(ctx context.Context, projectDirectory string, inventory migrate.Report, mappings []migrate.Mapping, state dependency.State, result realizeapp.Result, dryRun bool) (migrate.MigrationReport, error) {
	...
	report.FinalizationReady = len(report.EffectiveDiffs) == 0 && len(inventory.Ambiguous) == 0 && len(inventory.Unsupported) == 0 && !hasLossy(inventory) && !hasUncovered(inventory)
