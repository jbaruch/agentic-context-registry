// Package claudecode implements the Claude Code native adapter.
package claudecode
...
// Adapter renders Claude Code native layouts.
type Adapter struct{}
...
func (Adapter) Detect(_ context.Context, request adapter.DetectRequest) (adapter.Detection, error) {
	evidence, err := adapter.ExistingEvidence(request.Project,
		[]string{".claude/CLAUDE.md", settingsPath, "CLAUDE.md"}, []string{".claude/skills"})
	if err != nil {
	...
	for _, pkg := range sortedPackages(packages) {
		...
		for _, skill := range skills {
			...
			for _, file := range files {
				...
				items = append(items, adapter.PlanItem{
					...
					Target: path.Join(".claude/skills", name, relative), Kind: adapter.OutputGeneratedFile, Mode: mode,
				})
			}
		}
		for _, script := range sortedScripts(pkg.Manifest.Artifacts.Scripts) {
			...
			items = append(items, adapter.PlanItem{
				...
				Target: path.Join(".claude/scripts", name, adapter.SourceBasename(script.Path)), Kind: adapter.OutputGeneratedFile, Mode: 0o755,
			})
		}
		for _, hook := range sortedHooks(pkg.Manifest.Artifacts.Hooks) {
			...
			items = append(items,
				adapter.PlanItem{Owner: owner, Target: path.Join(".claude/hooks", name, adapter.SourceBasename(hook.Path)), Kind: adapter.OutputGeneratedFile, Mode: 0o755},
				adapter.PlanItem{Owner: owner, Target: settingsPath, Kind: adapter.OutputConfigMerge, Mode: 0o644},
			)
		}
	}
