...
| Finalization cannot prove every safety gate | 4 | `finalization_blocked` | Read `blockers[]` in the returned report: each names the path, the reason, and the remedy that clears it. Apply them, then run `acr migrate tessl --finalize --dry-run` | [Finalization](migration.md#finalization-and-recovery) |
...
| A `latest` or explicit `@TAG` mapping meets a repository GitHub cannot show (the release lookup and the repository itself both answer 404: missing, or private without usable credentials), a pinned version or requested tag GitHub returns as a draft or prerelease, or migration cannot complete another uncategorized operation | 1 | `migrate_failed` | For a private repository, run `gh auth login` or set a token that can read it; for a draft or prerelease tag, choose a stable one with `acr migrate tessl --map FROM=SOURCE@TAG`; for another cause, run `acr migrate tessl --dry-run --json`, inspect it, and retry | [Migration command](migration.md#command) |
| Consumer migration finds no supported native agent output in Tessl inventory | 1 | `migrate_failed` | Restore or generate this Tessl consumer's native output for `claude-code`, `codex`, or `cursor`, then rerun `acr migrate tessl` with the same options. Installed rule output under `.cursor/rules` or skill links under `.claude/skills`, `.codex/skills`, or `.cursor/skills` provide coverage; an `agents.yaml` edit alone does not, and consumer migration rejects `--agent` | [Migration command](migration.md#command) |
...
| A mapped release archive has no ACR manifest, unlike the no-release `source_not_published` case | 1 | `source_not_a_package` | In the producer, run `acr migrate tessl-plugin`, publish it, then retry migration | [Producer conversion](migration-producer.md) |
| The release lookup for a pinned Tessl version, `latest`, or an explicit `@TAG` answered 404, and GitHub then showed the repository while reporting no newest stable release, which a draft or prerelease never is | 1 | `source_not_published` | Complete [producer stage 0](migration-guide.md#stage-0-prepare-and-publish-the-producer): in the producer, run `acr migrate tessl-plugin`, publish a stable release with `acr publish`, then re-run `acr migrate tessl` | [Migration guide](migration-guide.md#stage-0-prepare-and-publish-the-producer) |
| Neither accepted release tag matches the installed Tessl version, and the repository publishes another stable release, cannot be shown by GitHub, or could not be inspected | 1 | `tessl_version_unavailable` | Supply the intended tag with `acr migrate tessl --map FROM=SOURCE@TAG`; for a private repository, run `gh auth login` first | [Package mappings](migration.md#package-mappings) |
...
| Existing ACR state selects different agents, or a mapped package's declaration or lock is changed, dropped, or re-resolved | 1 | `project_state_conflict` | Run `acr migrate tessl --dry-run --json`. For declaration conflicts, follow the named source/request and recovery: keep a GitHub request with the supplied `--map`, or explicitly `acr uninstall SOURCE` only if removal is intended. For agent or lock conflicts, review `agents.yaml` or `.agents/registry.lock`. Packages the mapping does not name are retained | [Migration guide](migration-guide.md) |

## Clean producer conversion

| Symptom | Exit | Error code | Remedy | Owning page |
| --- | --- | --- | --- | --- |
| Selected conversion agent fails | 1 | `agent_failed` | Inspect the agent diagnostic, then rerun `acr migrate tessl-plugin --acr-only --repository URL --agent codex --dry-run` after correcting it | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Conversion cannot use the selected agent | 1 | `agent_unavailable` | Select an available agent with `acr migrate tessl-plugin --acr-only --repository URL --agent codex --dry-run`, or omit `--agent` for deterministic conversion | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Credential files are found in semantic input | 1 | `credential_input` | Move credentials outside the selected package, workflow, and test scopes; retain only placeholders, then rerun `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| An agent proposes an invalid or out-of-scope edit | 1 | `invalid_agent_proposal` | Inspect the rejected proposal and retain the original files; rerun `acr migrate tessl-plugin --acr-only --repository URL --agent codex --dry-run` with corrected inputs | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Clean conversion has no explicit target repository | 1 | `invalid_options` | Supply the target with `acr migrate tessl-plugin --acr-only --repository https://github.com/OWNER/REPO --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| An incomplete conversion plan is passed for application | 1 | `invalid_plan` | Prepare a complete plan with `acr migrate tessl-plugin --acr-only --repository URL --dry-run`, resolve its blockers, then rerun without `--dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| A receipt or converted output differs from the recorded migration | 1 | `receipt_conflict` | Restore the original receipt and matching output or source checkout, then run `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Producer inputs change during planning or application | 1 | `source_changed` | Stop concurrent edits, inspect any rollback diagnostic, then rerun `acr migrate tessl-plugin --acr-only --repository URL --dry-run` from a stable checkout | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| A portable support file collides with existing content | 1 | `support_collision` | Reconcile the named support path, then rerun `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Another producer migration or interrupted transaction exists | 1 | `transaction_conflict` | Inspect the transaction backups and restore a consistent source checkout before rerunning `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| A producer path is reserved, escaping, linked, or a special file | 1 | `unsafe_path` | Select a confined authored package made of regular files and directories, then run `acr migrate tessl-plugin PATH --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Conversion cannot verify a regular file at mode 000 | 1 | `unsupported_file_mode` | Retain the original source, give the named file a verifiable mode, then run `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Producer semantics have no supported conversion | 1 | `unsupported_semantic_conversion` | Read every reported blocker and retain the source; resolve unsupported input before rerunning `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
| Semantic conversion has no skill tree for portable support files | 1 | `unsupported_support_files` | Provide a skill tree for the metadata and notices, then rerun `acr migrate tessl-plugin --acr-only --repository URL --dry-run` | [Clean conversion](migration-producer.md#clean-acr-only-conversion) |
...
| A selected import target does not exist and is not a host ACR writes | 4 | `unresolved_include` | Create the intended file or remove the import, then run `acr check`; an import naming `CLAUDE.md` or `AGENTS.md` resolves once the matching agent is selected, without editing the import | [Include graphs](preservation.md#include-graphs) |
