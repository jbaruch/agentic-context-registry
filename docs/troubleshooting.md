...
| The migration application cannot complete an uncategorized operation | 1 | `migrate_failed` | Run `acr migrate tessl --dry-run --json`, inspect the cause, and retry | [Migration command](migration.md#command) |
...
| A mapped repository has no ACR manifest | 1 | `source_not_a_package` | In the producer, run `acr migrate tessl-plugin`, publish it, then retry migration | [Producer conversion](migration-producer.md) |
