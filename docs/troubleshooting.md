...
| A mapped repository cannot be shown by GitHub (a release-resolution 404 that the repository itself also answers with 404, meaning missing or private without usable credentials), or migration cannot complete another uncategorized operation | 1 | `migrate_failed` | For a private repository, run `gh auth login` or set a token that can read it; for another cause, run `acr migrate tessl --dry-run --json`, inspect it, and retry | [Migration command](migration.md#command) |
...
| A mapped repository is readable but publishes no stable release, so a pinned Tessl version, `latest`, or an explicit `@TAG` has nothing to resolve; drafts and prereleases do not count | 1 | `source_not_published` | Complete [producer stage 0](migration-guide.md#stage-0-prepare-and-publish-the-producer): in the producer, run `acr migrate tessl-plugin`, publish a stable release with `acr publish`, then re-run `acr migrate tessl` | [Migration guide](migration-guide.md#stage-0-prepare-and-publish-the-producer) |
| Neither accepted release tag matches the installed Tessl version, in a repository that publishes some other stable release or that GitHub cannot show | 1 | `tessl_version_unavailable` | Supply the intended tag with `acr migrate tessl --map FROM=SOURCE@TAG`; for a private repository, run `gh auth login` first | [Package mappings](migration.md#package-mappings) |
