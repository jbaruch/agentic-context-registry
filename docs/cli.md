...
### Codex runtime support policy

Supported means a Codex CLI release that satisfies every check below on the platform it runs on. ACR records the observed version in `agentRuns[].runtimeVersion` and the boundary it used in `agentRuns[].isolation`.

| Check | What ACR requires | Refusal |
| --- | --- | --- |
| Platform | macOS with `/usr/bin/sandbox-exec`, or Linux with `bwrap` (bubblewrap) on `PATH` and a CA certificate bundle (`SSL_CERT_FILE` or a distribution location) | names the platform or the missing tool and how to install it |
| Executable | the native Codex binary on `PATH`, directly (release archive, Homebrew cask) or through the official npm launcher whose platform package vendors it; the launcher itself is never executed | names the path and the install options |
| Version | `codex --version` prints `codex-cli MAJOR.MINOR.PATCH`, at or above the minimum `0.153.2`; no ceiling, patch and minor differences never refuse | names the observed and minimum versions, or the unrecognized output |
| Positive control | the executable runs inside the boundary and answers `--version` | reports the boundary failure, with the user-namespace remedy on Linux |
| Negative control | a readable canary `AGENTS.md` is unreadable through the same boundary; on Linux an unbound directory cannot become the working directory, so a no-op wrapper is detected | reports that the boundary was not established |
| Flags | `codex exec --help` lists every flag ACR passes and the real parser accepts ACR's complete argument vector (`codex exec … -V`) | names the missing or rejected flag as an unsupported capability |
| Controls | `codex features list` advertises every required control (`shell_tool`, `apps`, `plugins`, `hooks`, `multi_agent`, `browser_use`, `computer_use`, `code_mode_host`, `skill_search`, `memories`, `remote_plugin` and the rest of the list in `internal/producerconvert/codex_runtime.go`) and `skip_host_skill_discovery`; with ACR's switches applied every disabled feature reads `false` except the runtime-forced `unified_exec`, and the skip reads `true` | names the control that is missing or was not honored as an unsupported capability |
...
Runtime probes were recorded on macOS and local Linux for these releases: `0.153.2` (2026-09-03), `0.154.0` (2026-09-09) and `0.155.0` (2026-09-17). The observed small macOS semantic conversion used 0.154.0; hosted conversion remains unverified. The table lives in `internal/producerconvert/codex_runtime.go` (`CodexVerifiedReleases`), the CI matrix in `.github/workflows/ci.yml` and the asset digests in `.github/scripts/install-codex.sh` name the same releases, and `internal/release` refuses a drift between them. Prerelease builds pass the version gate at or above the minimum but are not verified.

Renewal replaces the earlier exact-pin procedure: when a new Codex release ships, add it to the three places above with its published asset digests, let the `codex-runtime` CI job run the probes against the real binary on both platforms, and complete real conversion acceptance through the central subscription lane described in [testing](testing.md). A release that fails a probe is a scoped issue naming the probe, never a reason to relax the check; a release that passes needs no ACR code change to be used. Review the table monthly and before each CLI release.

Authentication reaches the provider one of two ways. Locally, ACR copies a regular `auth.json` from `CODEX_HOME` (default `~/.codex`) into a throwaway home under `${ACR_STATE_HOME:-<user-cache>/acr}/codex/` and removes it after the run; the configured home is never written, and a `warnings` entry reports when the runtime rewrote the copy so the operator can log in again if the service rotated the token. Noninteractively, `CODEX_API_KEY` in ACR's environment is forwarded to the provider's environment, never to its arguments; `OPENAI_API_KEY` is not consulted. Only `CODEX_API_KEY`, proxy variables and `SSL_CERT_FILE` are forwarded; the provider runs with a cleared environment, `HOME` and `CODEX_HOME` set to the throwaway home and `TMPDIR` set to ACR's private working directory. Raw event/file equality and schema validation precede report redaction. Initial and observed refreshed credential values are held privately across scopes and retries; credential-bearing decoded proposals, complete output and portable receipts refuse before writes. ACR rechecks that guard before application and sanitizes report/error copies. Every auth-capable child is followed by credential inspection; an uninspectable copied credential fails with captured output discarded. The original credential is never updated. These checks report `acr-credential-boundary/v1` observations, including whether refresh occurred. Coverage is limited to literal values actually observed; it does not infer intermediate credentials created and erased between observations. Key-shaped fragments are additionally redacted in diagnostic copies without changing proposal semantics.

Hosted subscription acceptance uses the existing central `jbaruch/coding-policy` `CODEX_AUTH_JSON` credential; ACR has only `FLEET_DISPATCH_TOKEN`. The central acceptance workflow is separate from routine reviewer dispatch. Its integration and actual hosted run must be verified before claiming Linux conversion acceptance; no ACR-local API key is required.
...
Implicit installs, `migrate tessl`, and `uninstall` check local authorization
before recovering an interrupted project journal; use an explicit
`acr install file:PATH` to authorize and repair. Without a pending journal,
an empty migration remains a no-op and uninstall can remove an unauthorized or
unavailable last local package.
