...
| `acr install [SOURCE[@VERSION]]` | Install one package, or reconcile declared dependencies when no source is supplied | Resolution in #5; realization in #7 |
| `acr realize` | Reapply locked packages without remote resolution | Transactional realization in #7 |
| `acr list` | List declared and resolved dependencies | Dependency state in #5 |
| `acr outdated` | Check `latest` dependencies without modifying project state | Remote resolution in #5 |
| `acr update [SOURCE]` | Update one dependency or all eligible dependencies | Resolution in #5; realization in #7 |
