...
Verified releases are the ones this contract was demonstrated on with the runtime probes on macOS and Linux ; an observed macOS semantic conversion used 0.154.0: `0.153.2` (2026-09-03), `0.154.0` (2026-09-09) and `0.155.0` (2026-09-17). The table lives in `internal/producerconvert/codex_runtime.go` (`CodexVerifiedReleases`), the CI matrix in `.github/workflows/ci.yml` and the asset digests in `.github/scripts/install-codex.sh` name the same releases, and `internal/release` refuses a drift between them. Prerelease builds pass the version gate at or above the minimum but are not verified.
