...
The [dual-publishing contract](publishing.md#dual-publishing) is the sole reference for maintaining both manifests and using one tag. The required order is producer conversion (#11) and then immutable ACR publication (#9). Until a stable release exists, mapping the repository fails during release resolution with `source_not_published`: GitHub answers for the repository but returns a 404 for its newest stable release, and drafts and prereleases do not count. Publish the producer in this stage; `gh auth login` is not the remedy. A repository GitHub cannot show at all, because it is missing or private without usable credentials, still reports `migrate_failed` with the authentication hint.
...
A mapping to a readable repository with no stable release yet fails during release resolution with `source_not_published`, whether the installed Tessl version is pinned, `latest`, or the mapping names an explicit `@TAG`: GitHub returned a 404 for the release while answering for the repository. Return to [stage 0](#stage-0-prepare-and-publish-the-producer) and publish the producer. A 404 for the repository itself is still reported as `migrate_failed`, and `gh auth login` remedies only that private-repository case. This differs from [`source_not_a_package`](troubleshooting.md#missing-source-mappings-and-migration), which means a release exists but its archive has no ACR manifest, and from [`tessl_version_unavailable`](troubleshooting.md#missing-source-mappings-and-migration), which means the repository publishes stable releases but none named after the installed Tessl version.
