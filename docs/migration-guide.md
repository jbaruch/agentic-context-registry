...
The result must report no unexpected effective differences. `--dry-run` writes nothing. ACR packages the project already installed are carried through untouched, so a gradual migration can run beside them. If nonempty pre-existing ACR state selects different agents, or requests a different version of a package the mapping names, the command exits `1` with `project_state_conflict`; run `acr migrate tessl --dry-run --json`, align or remove the disagreeing `agents.yaml` or `.agents/registry.lock`, and retry. If a package lacks repository evidence, `unmapped_package` writes nothing; continue to [stage 2](#stage-2-resolve-missing-mappings) for the mapping retry. Empty or absent state is synthesized, so prior initialization is neither required nor desirable.
