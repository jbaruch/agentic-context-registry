...
The [dual-publishing contract](publishing.md#dual-publishing) is the sole reference for maintaining both manifests and using one tag. The required order is producer conversion (#11) and then immutable ACR publication (#9). Until a stable release exists, mapping the repository fails during release resolution with `source_not_published`: the release lookup returns 404, and GitHub then shows the repository while reporting no newest stable release. A draft or prerelease is never that newest stable release, so a repository holding only those reaches the same code through a `latest` mapping or a pinned version none of them is tagged with; a pinned version or explicit `@TAG` that GitHub returns as a draft or prerelease is refused as a non-stable release tag with `migrate_failed` instead. Publish the producer in this stage; `gh auth login` is not the remedy. A repository GitHub cannot show at all, because it is missing or private without usable credentials, keeps its earlier code: `migrate_failed` with the authentication hint for a `latest` or explicit `@TAG` mapping, and `tessl_version_unavailable` for a pinned version mapped without a tag.
...
The result must report no unexpected effective differences. `--dry-run` writes nothing. ACR packages the project already installed are carried through untouched, so a gradual migration can run beside them. If nonempty pre-existing ACR state selects different agents, or requests a different version of a package the mapping names, the command exits `1` with `project_state_conflict`; run `acr migrate tessl --dry-run --json` and follow the diagnostic. A dependency request conflict names the source and both requests, with an explicit `--map` override to keep the current request. Keep the other original migration options when retrying. For agent-selection or lock conflicts, review the corresponding `agents.yaml` or `.agents/registry.lock` state before retrying. If a package lacks repository evidence, `unmapped_package` writes nothing; continue to [stage 2](#stage-2-resolve-missing-mappings) for the mapping retry. Empty or absent state is synthesized, so prior initialization is neither required nor desirable.
...
For many mappings, commit a YAML file and pass `--mapping-file PATH`. CLI `--map` values take precedence over the file, which takes precedence over repository evidence in the package manifest. `mapping_conflict`, `mapping_file_invalid`, `source_not_published`, `tessl_version_unavailable`, and `ambiguous_tessl_version` all write nothing; use the exact retry in [Troubleshooting](troubleshooting.md#missing-source-mappings-and-migration).

A mapping to a repository with no stable release yet fails during release resolution with `source_not_published`, whether the installed Tessl version is pinned, `latest`, or the mapping names an explicit `@TAG`: the release lookup returned 404, and GitHub then showed the repository while reporting no newest stable release. Return to [stage 0](#stage-0-prepare-and-publish-the-producer) and publish the producer. A 404 for the repository itself keeps the earlier code, `migrate_failed` with the authentication hint for `latest` and explicit `@TAG` mappings and `tessl_version_unavailable` for a pinned version mapped without a tag, and `gh auth login` remedies only that private-repository case. A pinned version or explicit `@TAG` that GitHub returns as a draft or prerelease is refused as a non-stable release tag with `migrate_failed` and never reaches the availability check. This differs from [`source_not_a_package`](troubleshooting.md#missing-source-mappings-and-migration), which means a release exists but its archive has no ACR manifest, and from [`tessl_version_unavailable`](troubleshooting.md#missing-source-mappings-and-migration), which means neither the pinned version nor its `v`-prefixed form is a stable release tag while the repository publishes another stable release, cannot be shown by GitHub, or could not be inspected.
