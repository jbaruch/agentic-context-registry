...
- **An argument container.** A `'` or `"` that opens at a token start and has a closing partner delimits one argument: only the position just inside the quote begins a reference, and whitespace inside the argument separates nothing. A quote with no partner opens nothing, so an odd quote cannot swallow the rest of a file. A backtick is a Markdown code span rather than an argument, so whitespace inside it still separates tokens and a backquoted command's arguments each begin a reference.
- **A label stack.** `](` opens a Markdown destination only where a `[` actually opened at a token start. Labels reset at each newline, because a Markdown link does not span lines.

A leading escape and one shell assignment prefix (`NAME=` or `--flag=`) are carried through ahead of the reference; the assignment name is a bounded run, so a URL query's `?next=` never qualifies. Every reference outside the two supported forms is preserved unchanged rather than rewritten into a path that resolves nowhere.
