...
A reference is rewritten only where it begins a token. A token begins at the start of the content, after whitespace, after one of the three quote characters, after a Markdown destination's `](`, and after an opening bracket that itself begins a token. That last clause is what separates a Markdown link destination, and a parenthesized package-root path in prose, from `archive(skills/<id>/...)` and `https://host/(skills/<id>/...)`: a bracket embedded in a word continues that word, so a URL, a query value and an unrelated filename stay whole. A leading escape and one shell assignment prefix (`NAME=` or `--flag=`) are carried through ahead of the reference; the assignment name is a bounded run, so a URL query's `?next=` never qualifies. Every reference outside the two supported forms is preserved unchanged rather than rewritten into a path that resolves nowhere.
