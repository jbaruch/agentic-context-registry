...
A reference is recognized as a whole token, never as a substring. Content is cut into tokens at the bytes a supported reference cannot contain — whitespace, the three quote characters, and the bracketing pairs — and a token qualifies only when a supported form starts at its beginning, after at most one leading escape and one shell assignment prefix (`NAME=` or `--flag=`). A URL, a URL query value, a filename with a punctuation or non-ASCII prefix, a path with a different leading segment, and a fragment of program syntax are therefore each a token that does not begin with a supported form, and each keeps its bytes. So does every reference outside the two supported forms: one ACR cannot map is preserved unchanged rather than rewritten into a path that resolves nowhere.
