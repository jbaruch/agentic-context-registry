...
The removal is a `structured-entry` splice inside the same transaction as every other edit. Every foreign member and unknown field survives byte for byte; in TOML the table header is removed with its fields, and a comment written between that table and the next is preserved.
