...
Consumer migration selects agents from the native output found by Tessl inventory, not from `--agent` or an `agents.yaml` edit alone. With no output for `claude-code`, `codex`, or `cursor`, it exits `1` with `migrate_failed` before applying migration changes. Restore or generate the Tessl consumer's native output for a supported agent, such as installed rule output under `.cursor/rules` or installed skill links under `.claude/skills`, `.codex/skills`, or `.cursor/skills`, then rerun `acr migrate tessl` with the same options. Inventory also recognizes `.claude/settings.json`, `.codex/config.toml`, and `.cursor/hooks.json`. Restore actual output for this consumer rather than creating empty evidence paths. A project with only uncovered agents has no supported migration target; `init`, `install`, `realize`, and `check` retain their own agent-selection options.
...
When `requested` is omitted, Tessl's `latest` remains `latest`. A pinned Tessl package version such as `1.1.12` must match exactly one stable GitHub release tag named either `1.1.12` or `v1.1.12`. An explicit `@REQUESTED` value is used as written. The mapped repository must already publish a stable GitHub release. When the release lookup for a pinned version, `latest`, or an explicit `@REQUESTED` answers 404 and GitHub then shows the repository while reporting no newest stable release, migration returns `source_not_published` and points to producer stage 0; a repository holding only drafts or prereleases reaches it through `latest` or through a pinned version none of them is tagged with, because a draft or prerelease is never the newest stable release. A pinned version or `@REQUESTED` tag that GitHub returns as a draft or prerelease is instead refused as a non-stable release tag with `migrate_failed`. The repository must also publish a valid `agent-plugin.yaml`; otherwise migration returns `source_not_a_package` and points to producer migration #11 and publishing #9.
