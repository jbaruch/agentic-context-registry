...
- **Codex conversion at scale.** The live lane converts the clean fixture and the untouched upstream fixtures it is given and runs their original tests on the converted trees; publication of a converted commit to a real GitHub repository, installation from that release on another machine, and conversion of the wider plugin fleet are not established by any lane here. A fixture the workflow sets to `skip` is excluded on purpose with the reason recorded beside the variable: `jbaruch/frequent-flyer-advocate` publishes through a job-level `uses:` call of a reusable workflow, a publisher shape the delivery-preservation validator keeps read-only, so every proposal that changes that job is refused before any write and the fixture proves the runtime, not the conversion. The upstream fixtures are pinned commits (`tesslio/good-oss-citizen` `f21fda887815af815979a4fea43a66eb5174ee3e`, `jbaruch/frequent-flyer-advocate` `142babbb1e2bebc798eb42128ac2466f21b5131d`) named in `.github/workflows/codex-live.yml`; refresh them there and in `cmd/acr/codex_live_test.go` together when a newer untouched baseline is verified. A fresh conversion may produce a different proposal, so "the same converted commit locally and in CI" means one recorded commit consumed in both places, never two conversions expected to agree byte for byte.
