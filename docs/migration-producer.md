...
Semantic conversion checks publication policy against the original workflow before deterministic translation. Retained jobs keep their non-execution fields. Each original publisher-step condition must stay on its corresponding retained publisher execution with the same YAML presence, type and value and distinct occurrences in source order. A guarded standalone `run: acr publish .` is supported; a guard on checkout or validation cannot substitute. Step-guarded job/workflow retirement and reusable replacement refuse before writes, without transferring guards across scopes. Whole publisher-job deletion also requires the original infrastructure-only shape described in [the CLI contract](cli.md#semantic-producer-conversion). Paid-score-only retirement has separate disclosure and reference requirements and cannot excuse dropping publisher policy.

The supported pinned reusable rewrite preserves original policy and separately checks explicit effective caller permissions. Job `permissions`, when present, overrides workflow `permissions`; explicit job maps do not inherit omitted entries. `write-all` or literal `contents: write` is sufficient. `read-all`, empty maps, maps omitting `contents`, `contents: read`/`none` and unsupported representations refuse. Conversion never changes or elevates permissions. Omitting both scopes remains structurally accepted with repository defaults and hosted capability unverified. Sufficient permissions do not establish trigger/tag compatibility or hosted publication. The existing main-triggered, false-job-guard preservation case remains supported with sufficient permissions; a false guard cannot excuse explicit insufficiency.
...
The deterministic converter does **not** complete issue #117's untouched-Good-OSS-Citizen acceptance on its own. Custom operations that read/write Tessl configuration, declare or resolve dependencies, build installed paths dynamically, or change Tessl-backed review policy return `unsupported_semantic_conversion` with actionable paths before writes. The converter detects literal Tessl operations; it cannot infer arbitrary program behavior or port obfuscated/dynamically synthesized code. Historical Tessl mentions and research URLs are not by themselves executable dependencies.

Initial macOS semantic conversion evidence used Codex 0.154.0. Initial GOC and FFA attempts did not complete; no generated upstream commit has been published. The exact fleet publisher contract below is now translated deterministically, but remaining semantic references and full local/hosted journeys still require real acceptance runs.
...
GOC's original install-gate reads its installed plugin version, writes dependency state while preserving pins, installs templates and coordinates rollback. Its preflight/commit helpers and review workflows also depend on Tessl state and policy. Substituting path strings cannot preserve those semantics; the semantic route proposes complete helpers, and ACR structurally validates proposals before writing; acceptance separately executes the original command surface and must independently verify each necessary generated framework-fixture adaptation against immutable upstream behavioral obligations. A deterministic GOC refusal proves this boundary, not successful GOC migration, and unknown reusable publisher contracts still refuse.
...
The exact fleet caller `jbaruch/coding-policy/.github/workflows/publish-plugin.yml@af116ebf18a7c46a672bf176064908736bc8ac28` has a deterministic root-package translation. Its literal existing `pre-publish-script` becomes a separate gate job, optionally using the original literal Python version, and the publisher requires that gate via `needs`. The gate script and independent tests/review retain their bytes. The output calls `jbaruch/agentic-context-registry/.github/workflows/publish-package.yml@d3bc96b33b42293aecd1702c04aa94513a3dab1b` with `path: .` and `acr-version: v0.1.6`. Paid Tessl review is explicitly retired with no equivalent score. Unknown revisions, inherited/unknown secrets, input expressions, unsafe or missing gate paths, and unsupported policy refuse. A new callee revision requires renewed contract review; the converter never fetches or interprets arbitrary reusable workflows.
