...
The semantic route (`--agent codex`) runs on current Codex releases on macOS and Linux under the [runtime support policy](cli.md#codex-runtime-support-policy). The untouched-upstream journeys live in the dispatch-only `codex-live` workflow and `cmd/acr/codex_live_test.go` (see [testing](testing.md#boundaries)): the original tests run before conversion, the deterministic converter must refuse, a real release converts, nothing outside the reported delta moves, the converted commit is recorded and consumed by the publication acceptance test, and the original tests run again on the converted tree. What that route has established so far, from untouched copies at pinned commits: the clean producer fixture converts end to end; every Codex turn for `jbaruch/frequent-flyer-advocate` (`142babbb`) completes, but the delivery proposal is refused before any write because its publisher is a job-level `uses:` call of a reusable workflow, which the [delivery-preservation rules](cli.md#semantic-producer-conversion) keep read-only; for `tesslio/good-oss-citizen` (`f21fda88`, package `plugins/good-oss-citizen`) the runtime and instruction scopes completed and the delivery scope has not yet completed because the account reached its usage limit. Neither upstream conversion is complete, and no converted commit has been published to a real repository.
...
GOC's original install-gate reads its installed plugin version, writes dependency state while preserving pins, installs templates and coordinates rollback. Its preflight/commit helpers and review workflows also depend on Tessl state and policy. Substituting path strings cannot preserve those semantics; the semantic route proposes complete helpers, and ACR validates them against the original tests before writing. A deterministic GOC refusal proves this boundary, not successful GOC migration, and a publisher that calls a reusable workflow at the job level still has no supported conversion.
