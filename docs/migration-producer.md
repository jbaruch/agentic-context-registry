...
GOC's original install-gate reads its installed plugin version, writes dependency state while preserving pins, installs templates and coordinates rollback. Its preflight/commit helpers and review workflows also depend on Tessl state and policy. Substituting path strings cannot preserve those semantics; the semantic route proposes complete helpers, and ACR structurally validates proposals before writing; acceptance separately executes the unchanged original tests. A deterministic GOC refusal proves this boundary, not successful GOC migration, and unknown reusable publisher contracts still refuse.
